...
from .h1_utils import FootStepGeometry, SimpleLineGeometry, VelCommandGeometry, smart_sort
...
class H1Controller(LeggedRobot):
    ...
    def _init_buffers(self):
        ...
        self.right_hip_pos = self.rigid_body_state[:, self.rigid_body_idx['right_hip_yaw_link'], :3]
        self.left_hip_pos = self.rigid_body_state[:, self.rigid_body_idx['left_hip_yaw_link'], :3]
    ...
    def _reset_system(self, env_ids):
        ...
        # * Step commands
        self.step_commands[env_ids, 0] = self.env_origins[env_ids] + torch.tensor([0., 0.163*0.9, 0.], device=self.device) # right foot initializatoin
        self.step_commands[env_ids, 1] = self.env_origins[env_ids] + torch.tensor([0., -0.163*0.9, 0.], device=self.device) # left foot initializatoin
    ...
    def _update_robot_states(self):
        ...
        self.ankle_vel_history[:,0,:naxis] = self.rigid_body_state[:, self.rigid_body_idx['right_ankle_roll_link'], 7:10]
        self.ankle_vel_history[:,1,naxis:] = self.ankle_vel_history[:,1,:naxis]
        self.ankle_vel_history[:,1,:naxis] = self.rigid_body_state[:, self.rigid_body_idx['left_ankle_roll_link'], 7:10]
