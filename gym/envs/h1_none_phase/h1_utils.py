...
class ArrowGeometry(LineGeometry):
    def __init__(self, base: torch.Tensor, lin_vel_command: torch.Tensor, color: Tuple):
        ...
        arrow_neck = (
            arrow_head_ratio * base + (1 - arrow_head_ratio) * lin_vel_command_in_world
        )
        arrow_tip_width = 0.1
        unit_rotation_matrix = torch.tensor(
            [
        ...
        unit_z = torch.tensor([0.0, 0.0, 1.0], device=self.device)
        axis = torch.cross(unit_z, lin_vel_command)
        axis = axis / torch.norm(axis)
        angle = torch.acos(
            torch.inner(unit_z, lin_vel_command) / torch.norm(lin_vel_command)
        )
...
class FootStepGeometry(LineGeometry):
    def __init__(
        self, step_location: torch.Tensor, step_orientation: torch.Tensor, color: Tuple
    ):
        """
        ...
        step_location = torch.tensor(
            [*step_location, 0.0], device=self.device
        )  # (x, y, 0)

        angle = torch.tensor([60])  # angle between each point on circle's circumference
        num_lines = int(360 / angle.item()) + 1
        verts = np.empty((num_lines, 2), gymapi.Vec3.dtype)
        unit_rotation_matrix_func = lambda angle: torch.tensor(
            [
                [
        ...
        # Foot step structure
        offset = torch.tensor([0.1, 0.0, 0.0], dtype=torch.float, device=self.device)
...
class CircleGeometry(LineGeometry):
    def __init__(self, center: torch.Tensor, radius: torch.Tensor, color: Tuple):
        ...
        # Circle structure
        offset = torch.tensor(
            [radius.item(), 0.0, 0.0], dtype=torch.float, device=self.device
        )
