...
class Logger:
    ...
    def initialize_buffers(self, num_envs, reward_keys):
        self.current_episode_return = {
            name: torch.zeros(
                num_envs, dtype=torch.float, device=self.device, requires_grad=False
            )
        ...
        self.avg_return_buffer = {
            name: deque(maxlen=self.avg_window) for name in reward_keys
        }
        self.avg_length_buffer = deque(maxlen=self.avg_window)
        self.mean_rewards = {"Episode/" + name: 0.0 for name in reward_keys}
    ...
    def update_episode_buffer(self, dones):
        ...
        for name in self.current_episode_return.keys():
            self.avg_return_buffer[name].extend(
                self.current_episode_return[name][terminated_ids].cpu().numpy().tolist()
            )
            self.current_episode_return[name][terminated_ids] = 0.0
    ...
    def calculate_reward_avg(self):
        ...
        self.mean_rewards = {
            "Episode/"
            + name: mean(self.avg_return_buffer[name]) / self.max_episode_length_s
            for name in self.current_episode_return.keys()
        }
    ...
    def configure_local_files(self, save_paths):
        ...
        for save_path in save_paths:
            if save_path["type"] == "file":
                ...
                shutil.copy2(
                    save_path["source_file"], save_dir + save_path["target_dir"]
                )
