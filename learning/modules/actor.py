...
class Actor(nn.Module):
    ...
    def get_scaled_ractions_and_log_prob(self, observations, only_actions=False):
        ...
        actions_scaled = (
            self.actions_range_center + self.actions_range_radius * actions_normalized
        )

        if only_actions:
            return actions_scaled
        else:
            actions_log_prob = self.distribution.log_prob(actions).sum(
                dim=-1
    ...
    def export(self, path):
        ...
        if self._normalize_obs:
            ...
            model = NormalizedActor(
                copy.deepcopy(self.mean_NN), copy.deepcopy(self.obs_rms)
            ).to("cpu")
